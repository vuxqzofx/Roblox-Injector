...
class CK_VISIBLE_PUBLIC CkPrivateKey  : public CkClassWithCallbacks
{
	...
	static CkPrivateKey *createNew2(int progLang);
	...
	// The default value is 3des . Possible choices also include aes128 , aes192 , and
	// aes256 . All of the encryption algorithm choices use CBC mode.
	...
	// Retrieves the private key in binary DER format. If pkcs1 is true, it uses the
	// PKCS1 format; if false, it uses the PKCS8 format. If password is non-empty and the
	// PKCS8 format is specified, an encrypted PKCS8 key is returned. The private key
	// is returned in bd.
	bool GetPkcsBd(bool pkcs1, const char *password, CkBinData &bd);


	// Applications should instead call ToPublicKey .
	// 
	...
	// Saves the private key to an unencrypted binary PKCS8 format file. The path is an
	// absolute or relative file path conforming to the operating system. For example,
	// C:\temp\myKey.key .
	// 
	// Starting in Chilkat v10.0.0, on MacOS, the path can be formatted as
	// keychain:<label> to save the private key to the Apple Keychain.
	// 
	...
	// Returns in pubKey the public key portion of the private key.
	bool ToPublicKey(CkPublicKey &pubKey);
