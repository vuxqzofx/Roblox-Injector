...
class CK_VISIBLE_PUBLIC CkJwsW  : public CkWideCharBase
{
	...
	// Returns in json the protected header of the signature at the specified index. The
	// first signature is at index 0. Typically, a JWS has a single signature at index
	// 0.
	bool GetProtectedH(int index, CkJsonObjectW &json);

	// Applications should instead call GetProtectedH .
	// 
	...
	// Returns in json the original unprotected header of the signature at the
	// specified index. The first signature is at index 0. Typically, a JWS has a single
	// signature at index 0.
	bool GetUnprotectedH(int index, CkJsonObjectW &json);

	// Applications should instead call GetUnprotectedH .
	// 
	...
	// representation of the MAC key bytes. The encoding is an encoding, such as base64url
	// , base64 , hex , etc. The 1st signature is at index 0. (Most JWSs only contain a
	// single signature.)
	bool SetMacKey(int index, const wchar_t *key, const wchar_t *encoding);

	// Sets the MAC key to be used for creating or validating a signature for the
	// algorithms HS256 , HS384 , or HS512 . The key contains the binary bytes of the
	// MAC key for the Nth signature. The 1st signature is at index 0.
