...
class CK_VISIBLE_PUBLIC CkStream  : public CkClassWithCallbacks
{
	...
	static CkStream *createNew2(int progLang);
	...
	// 1. 0>
	//     2. No failure (success)
	//     3. Timeout, or no data is immediately available for a polling read.
	//     4. Aborted by an application callback.
	//     5. Already at end-of-stream.
	//     6. Fatal stream error.
	//     7. Out-of-memory error (this is very unlikely).
	...
	// 1. 0>
	//     2. No failure (success)
	//     3. Timeout, or unable to immediately write when the WriteTimeoutMs is 0.
	//     4. Aborted by an application callback.
	//     5. The stream has already ended.
	//     6. Fatal stream error.
	//     7. Out-of-memory error (this is very unlikely).
	...
	// The encoding argument indicates the encoding, which can be base64 , hex , or any of
	// the multitude of encodings indicated in the link below.
	...
	// according to the StringCharset property. For example, if utf-8 bytes are
	// expected, then StringCharset should be set to utf-8 prior to calling ReadString.
