...
class CK_VISIBLE_PUBLIC CkNtlm  : public CkMultiByteBase
{
	...
	static CkNtlm *createNew2(int progLang);
	...
	// The mapping of letters to bit values are as follows:0x01 - A0x02 - B0x04 - C0x10
	// - D0x20 - E0x40 - F0x80 - G0x200 - H0x400 - I0x800 - J0x1000 - K0x2000 - L0x8000
	// - M0x10000 - N0x20000 - O0x40000 - P0x80000 - Q0x100000 - R0x400000 - S0x800000
	// - T0x2000000 - U0x20000000 - V0x40000000 - W0x80000000 - X The default Flags
	// value has the following flags set: NegotiateUnicode, NegotiateOEM,
	// RequestTarget, NegotiateNTLMKey, NegotiateAlwaysSign, NegotiateNTLM2Key. The
	// corresponds to the string ABCHMQ .
	// 
	...
	// If the A flag is unset, then Unicode strings are not used internally in the NTLM
	// messages. Strings are instead represented using the OEM code page (i.e. charset,
	// or character encoding) as specified here. In general, given that the Flags
	// property should rarely be modified, and given that the A flag is set by default
	// (meaning that Unicode is used), the OemCodePage property will not apply. The
