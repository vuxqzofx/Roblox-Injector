...
class CK_VISIBLE_PUBLIC CkAiW  : public CkClassWithCallbacksW
{
	...
	// References:
	// 1: #Provider
	const wchar_t *apiKeyHeader(void);
	// For custom Provider 's, use this field name for the API key in the HTTP header,
	// but only if the API key is to be placed in a custom header field. If the API key
	// is to be provided in an "Authorization: Bearer" header, then leave this property
	// empty.
	...
	// References:
	// 1: #Provider
	void put_ApiKeyHeader(const wchar_t *newVal);
	...
	// References:
	// 1: #Provider
	void put_ApiSpec(const wchar_t *newVal);
	...
	// References:
	// 1: #Ask
	bool SetAskParams(CkJsonObjectW &json);
	...
	bool SetProviderParams(CkJsonObjectW &json);
