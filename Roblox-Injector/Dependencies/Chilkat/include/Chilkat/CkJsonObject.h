...
class CK_VISIBLE_PUBLIC CkJsonObject  : public CkMultiByteBase
{
	...
	static CkJsonObject *createNew2(int progLang);
	...
	// Properties
	// ----------------------
	// Sets the delimiter char for JSON paths. The default value is . . To use Firebase
	// style paths, set this property to / . (This is a string property that should
	// have a length of 1 char.)
	...
	// Important: The name is the member name, it is not a JSON path.
	// 
	bool AppendArray2(const char *name, CkJsonArray &jarr);
	...
	// Important: The name is the member name, it is not a JSON path.
	// 
	bool AppendObject2(const char *name, CkJsonObject &jsonObj);
	...
	bool ArrayAt2(int index, CkJsonArray &jarr);
	...
	// Sets jarr to reference the JSON array found at jsonPath.
	bool ArrayOf2(const char *jsonPath, CkJsonArray &jarr);
	...
	// Updates jsonObj to reference this JSON object.
	bool CopyRef(CkJsonObject &jsonObj);
...
const char *emit(void);
	...
	// Recursively searches this JSON subtree for an object with a specified member,
	// name. If the function is called at the root of the JSON document, it searches
	// the entire document. It returns the first match found, or _NULL_ if no match is
	// found.
	// The caller is responsible for deleting the object returned by this method.
	CkJsonObject *FindObjectWithMember(const char *name);


	// Recursively searches this JSON subtree for an object containing a specified
	// member, name. If initiated at the root, it searches the entire document. It
	// returns true and stores the result in jsonObj if the member is found; otherwise,
	// it returns false.
	bool FindObjectWithMember2(const char *name, CkJsonObject &jsonObj);


	// Locate a JSON record in an array where a specific field matches a given value or
	// pattern. Refer to the example below for clarification.
	// The caller is responsible for deleting the object returned by this method.
	...
	// Locate a JSON record in an array where a specific field matches a given value or
	// pattern. Refer to the example below for clarification.
	bool FindRecord2(const char *arrayPath, const char *relPath, const char *value, bool caseSensitive, CkJsonObject &jsonObj);
	...
	// Returns the root of the JSON document, accessible from any JSON object within
	// it. The entire document stays in memory as long as at least one JSON object is
	// referenced by the application. Once the last reference is removed, the document
	// is automatically deallocated.
	// The caller is responsible for deleting the object returned by this method.
	CkJsonObject *GetDocRoot(void);


	// Returns in docRoot the root of the JSON document. The entire document stays in
	// memory as long as at least one JSON object is referenced by the application.
	// Once the last reference is removed, the document is automatically deallocated.
	bool GetDocRoot2(CkJsonObject &docRoot);
	...
	// Returns a copy of this JSON object in json.
	bool MakeCopy(CkJsonObject &json);
	...
	// 0.
	bool ObjectAt2(int index, CkJsonObject &jsonObj);
	...
	bool ObjectOf2(const char *jsonPath, CkJsonObject &jsonObj);
