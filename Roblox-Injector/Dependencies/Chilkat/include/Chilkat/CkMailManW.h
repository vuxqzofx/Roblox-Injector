...
class CK_VISIBLE_PUBLIC CkMailManW  : public CkClassWithCallbacksW
{
	...
	// be set to the string default to cause the component to use the current logged-on
	...
	// to each recipient in the list one at a time, with the To header field containing
	// the email address of the recipient. If false, emails will contain in the To
	// header field, and are sent to 100 BCC recipients at a time. As an example, if
	// your distribution list contained 350 email addresses, 4 emails would be sent,
	// the first 3 having 100 BCC recipients, and the last email with 50 BCC
	...
	//     2. Failed A general failure not covered by any of the other possible
	//     keywords.
	...
	//     5. SomeBadRecipients The AllOrNone property is true, and some recipients
	//     were rejected by the SMTP server.
	//     6. Aborted The application aborted the method.
	//     7. NoFrom The failed to provide a FROM address.
	//     8. FromFailure The SMTP replied with an error in response to the MAIL FROM
	//     command.
	//     9. NoCredentials The application did not provide the required credentials,
	//     such as username or password.
	//     10. AuthFailure The login (authentication) failed.
	//     11. DataFailure The SMTP replied with an error in response to the DATA
	//     command.
	//     12. NoSmtpHostname The application failed to provide an SMTP hostname or IP
	//     address.
	//     13. StartTlsFailed Failed to convert the TCP connection to TLS via STARTTLS.
	//     14. ConnectFailed Unable to establish a TCP or TLS connection to the SMTP
	//     server.
	//     15. GreetingError The SMTP server immediately responded with an error status
	//     in the intial greeting.
	//     16. ConnectionLost The connection to the SMTP server was lost at some point
	//     during the method call.
	//     17. Timeout A timeout occurred when reading or writing the socket
	//     connection.
	//     18. RenderFailed A failure occurred when rendering the email. (Rendering the
	//     email for sending includes tasks such as signing or encrypting.)
	//     19. NotUnlocked The UnlockBundle method was not previously called on at
	//     least one instance of the mailman object.
	//     20. InternalFailure An internal failure that should be reported to Chilkat
	//     support.
	...
	// This is the email address passed in the MAIL FROM command in the SMTP protocol.
	// It specifies the envelope sender address — the address that will receive bounces
	// (delivery failure messages), and is the originator of the SMTP transaction.
	//     * SMTP servers may reject MAIL FROM addresses based on DNS or policy (SPF
	//     records)
	//     * Bounce messages (delivery failure notifications) are sent to the address
	//     in MAIL FROM
	// 
	// This property is empty by default. If this property is left empty, which is
	// typical, the email address in the From MIME header of the email is used as the
	// MAIL FROM address.
	// 
	void get_SmtpMailFrom(CkString &str);
	// This is the email address passed in the MAIL FROM command in the SMTP protocol.
	// It specifies the envelope sender address — the address that will receive bounces
	// (delivery failure messages), and is the originator of the SMTP transaction.
	//     * SMTP servers may reject MAIL FROM addresses based on DNS or policy (SPF
	...
	// MAIL FROM address.
	// 
	const wchar_t *smtpMailFrom(void);
	// This is the email address passed in the MAIL FROM command in the SMTP protocol.
	// It specifies the envelope sender address — the address that will receive bounces
	// (delivery failure messages), and is the originator of the SMTP transaction.
	//     * SMTP servers may reject MAIL FROM addresses based on DNS or policy (SPF
	...
	// MAIL FROM address.
	// 
	void put_SmtpMailFrom(const wchar_t *newVal);
	...
	// SmtpPassword properties may be set to the string default to cause the component
	// to use the current logged-on credentials (of the calling process) for
	// authentication.
	// 
	...
	//     * SmtpLoginAnsi - Introduced in v9.5.0.97. If the SMTP login and/or password
	//     contains non-us-ascii chars, some SMTP servers expect the utf-8 encoding,
	//     whereas others expect ANSI. Historically, Chilkat passed the login/password
	//     using the ANSI encoding. Starting in v9.5.0.97, Chilkat switched to using the
	//     utf-8 encoding. This uncommon option can be used to revert back to the old
	//     behavior of sending the ANSI byte representation.
	...
	// known as PKCS12 or .p12). The password is the password to the PFX.
	...
	// of the emails, a QUIT message must be sent and the POP3 session ended. This will
	...
	// email, a QUIT message must be sent and the POP3 session ended. This will happen
	// automatically when the ImmediateDelete property equals true, which is the
	// default. If ImmediateDelete equals false, then the Pop3EndSession method can
	// be called to send the QUIT and end the session (i.e. disconnect.)
	// 
	// Note: When making multiple calls to a Delete* method, it's best to turn off
	// ImmediateDelete, and then manually call Pop3EndSession to finalize the
	// deletions.
	...
	// any in stUidls. To finalize deletions, a QUIT message must be sent and the POP3
	// session closed. This occurs automatically when the ImmediateDelete property is
	// set to true (default setting). If ImmediateDelete is false, use the
	// Pop3EndSession method to manually send the QUIT message and disconnect.
	// 
	// Note: When making multiple Delete* method calls, it's advisable to set
	// ImmediateDelete to false and manually execute Pop3EndSession to complete
	// deletions. Any method that requires server communication will automatically
	// re-establish a session according to the current property settings.
	// 
	// 
	// References:
	// 1: #ImmediateDelete
	// 2: #Pop3EndSession
	bool DeleteUidlSet(CkStringTableW &stUidls);

	// Creates an asynchronous task to call the DeleteUidlSet method with the arguments
	// provided.
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *DeleteUidlSetAsync(CkStringTableW &stUidls);

	// Retrieves all emails from the POP3 server. If keepOnServer is true, emails remain on
	// the server. If headersOnly is true, only the headers and the first numBodyLines lines of each
	// email are downloaded, excluding attachments. Otherwise, the entire emails with
	// attachments are downloaded. The emails are stored in bundle.
	// 
	// Note: keepOnServer only applies when downloading full emails (not headers-only).
	// Downloading headers-only will not cause the email to be deleted from the server,
	// regardless of keepOnServer.
	// 
	bool FetchAll(bool keepOnServer, bool headersOnly, int numBodyLines, CkEmailBundleW &bundle);

	// Creates an asynchronous task to call the FetchAll method with the arguments
	// provided.
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *FetchAllAsync(bool keepOnServer, bool headersOnly, int numBodyLines, CkEmailBundleW &bundle);
	...
	// Retrieves either the complete email or just the header along with the first N
	// lines of the message body from the POP3 server specified by uidl, without
	// deleting the email from the server.
	bool FetchByUidl(const wchar_t *uidl, bool headerOnly, int numBodyLines, CkEmailW &email);

	// Creates an asynchronous task to call the FetchByUidl method with the arguments
	// provided.
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *FetchByUidlAsync(const wchar_t *uidl, bool headerOnly, int numBodyLines, CkEmailW &email);
	...
	// If a partial email (header-only) was retrieved, this method will download and
	// return the full email from the server using the partial email as an argument.
	bool FetchFull(CkEmailW &partialEmail, CkEmailW &fullEmail);

	// Creates an asynchronous task to call the FetchFull method with the arguments
	// provided.
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *FetchFullAsync(CkEmailW &partialEmail, CkEmailW &fullEmail);
	...
	// Retrieves an email by its message number and returns the MIME bytes. Note:
	// Message numbers are unique to each POP3 session. For instance, if there are 10
	// messages in the maildrop, they will be numbered 1 through 10. If message 1 is
	// deleted and a new POP3 session is started, the remaining messages will be
	// renumbered from 1 to 9. Please note that a POP3 connection must be established
	// beforehand. This can be done by explicitly calling Pop3BeginSession or through
	// other methods that implicitly start the session. This method does not initiate a
	// POP3 session automatically.
	...
	// Retrieves an email by its message number and provides the MIME source in bd.
	// Note: Message numbers are unique to each POP3 session. For instance, if there
	// are 10 messages in the maildrop, they will be numbered 1 through 10. If message
	// 1 is deleted and a new POP3 session is started, the remaining messages will be
	// renumbered from 1 to 9. Please note that a POP3 connection must be established
	// beforehand. This can be done by explicitly calling Pop3BeginSession or through
	// other methods that implicitly start the session. This method does not initiate a
	// POP3 session automatically.
	bool FetchMimeByMsgnumBd(int msgnum, CkBinDataW &bd);

	// Creates an asynchronous task to call the FetchMimeByMsgnumBd method with the
	// arguments provided.
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *FetchMimeByMsgnumBdAsync(int msgnum, CkBinDataW &bd);
	...
	// This deprecated method will be removed in a future major release of Chilkat.
	// MIME can potentially include non-encoded binary data and mixed character
	// encodings, so downloading emails as a simple MIME string often requires
	// processing and modifications, making it impractical. Instead, applications
	// should use FetchMimeBd or methods that download emails to email objects.
	// 
	// This method downloads emails from the POP3 server for each UIDL in uidlArray and
	// returns an object containing the collection of downloaded MIME strings.
	// 
	// The caller is responsible for deleting the object returned by this method.
	...
	// Retrieves a complete message or header by its message number and stores it in
	// msgNum. The first email has a message number of 1. Messages fetched by this method
	// remain on the server.
	// 
	// Note: Message numbers are unique to each POP3 session.
	// 
	bool FetchOne(bool headerOnly, int numBodyLines, int msgNum, CkEmailW &email);

	// Creates an asynchronous task to call the FetchOne method with the arguments
	// provided.
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *FetchOneAsync(bool headerOnly, int numBodyLines, int msgNum, CkEmailW &email);

	// Retrieves a specified range of emails from the POP3 server. If keepOnServer is true,
	// the emails remain on the server. If headersOnly is true, only the headers and the
	// first numBodyLines lines of each email are downloaded, without attachments. Otherwise,
	// the entire emails, including attachments, are downloaded. The range of emails to
	// download is determined by startIndex and endIndex. The GetMailboxCount method returns the
	// total number of emails in the POP3 mailbox, with the first email at index 0. The
	// downloaded emails are stored in bundle.
	// 
	// Note: keepOnServer only applies when downloading full emails (not headers-only).
	// Downloading headers-only will not cause the email to be deleted from the server,
	// regardless of keepOnServer.
	// 
	bool FetchRange(bool keepOnServer, bool headersOnly, int numBodyLines, int startIndex, int endIndex, CkEmailBundleW &bundle);

	// Creates an asynchronous task to call the FetchRange method with the arguments
	// provided.
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *FetchRangeAsync(bool keepOnServer, bool headersOnly, int numBodyLines, int startIndex, int endIndex, CkEmailBundleW &bundle);
	...
	// 
	// POP3 UIDLs (Unique ID Listings) are persistent, unique identifiers assigned by a
	// mail server to each email message in a mailbox. Unlike message numbers, which
	// can change between sessions, UIDLs remain consistent as long as the message
	// exists, allowing email clients to track which messages have already been
	// downloaded—even across multiple sessions—without re-fetching the same emails.
	// 
	bool FetchUidls(CkStringTableW &uidls);

	// Creates an asynchronous task to call the FetchUidls method with the arguments
	// provided.
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *FetchUidlsAsync(CkStringTableW &uidls);

	// Fetches the email headers or full emails from the POP3 server whose UIDL is
	// present in the uidls. If headersOnly is true, only the headers and the first numBodyLines
	// lines of each email are downloaded, excluding attachments. Otherwise, the entire
	// emails with attachments are downloaded. The emails are stored in bundle. The
	// downloaded emails are not deleted from the server.
	bool FetchUidlSet(CkStringTableW &uidls, bool headersOnly, int numBodyLines, CkEmailBundleW &bundle);

	// Creates an asynchronous task to call the FetchUidlSet method with the arguments
	// provided.
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *FetchUidlSetAsync(CkStringTableW &uidls, bool headersOnly, int numBodyLines, CkEmailBundleW &bundle);

	// This method is deprecated. Applications should instead call FetchAll .
	// 
	// Retrieves all emails from the POP3 server, limiting the body to the first numBodyLines
	// lines and excluding attachments. The returned emails are valid objects with
	// truncated bodies and no attachments.
	// 
	// The caller is responsible for deleting the object returned by this method.
	...
	// This method is deprecated and applications should instead call SmtpRecipientsLog
	// 
	// Returns a list of email addresses rejected by the SMTP server from the last call
	// of an email-sending method, like SendEmail. Invalid email addresses identified
	// by the SMTP server are stored in a bad email addresses list, while valid ones
	// are stored in a good email addresses list within the object. Both lists reset
	// automatically at the beginning of each new mail-sending call, allowing you to
	// track which addresses were accepted or rejected.
	// 
	// Note: A the time of the SMTP session, the SMTP server can only verify the
	// validity of an email address within its own domain because it has access to its
	// internal user database or mailbox configuration. Addresses outside its domain,
	// are implicitly accepted and the validity of the email address won't be
	// discovered until arrives at the destination mail server.
	// 
	// The caller is responsible for deleting the object returned by this method.
	...
	// 
	// If a partial email (header-only) is retrieved using GetHeaders or GetAllHeaders,
	// this method will download and return the full email from the server using the
	// partial email as an argument.
	// 
	// The caller is responsible for deleting the object returned by this method.
	...
	// This method is deprecated and applications should instead call SmtpRecipientsLog
	// 
	// Returns a list of email addresses accepted by the SMTP server from the last call
	// of an email-sending method, like SendEmail. Invalid email addresses identified
	// by the SMTP server are stored in a bad email addresses list, while valid ones
	// are stored in a good email addresses list within the object. Both lists reset
	// automatically at the beginning of each new mail-sending call, allowing you to
	...
	// If the current connection is SSL/TLS, this method returns the digital
	// certificate of the SMTP or POP3 server specified by useSmtp. The certificate is
	// returned in cert.
	bool GetServerCert(bool useSmtp, CkCertW &cert);
	...
	// This deprecated method will be removed in a future Chilkat major version.
	// Applications should instead call the email object's LoadEml method.
	// 
	...
	// Loads a .mbx file containing emails and returns in bundle. If a Filter is present,
	// only emails that match the filter are returned.
	bool LoadMbxFile(const wchar_t *mbxPath, CkEmailBundleW &bundle);

	// This deprecated method will be removed in a future Chilkat major version.
	// Applications should instead call the email object's SetFromMimeText method.
	// 
	...
	// This deprecated method will be removed in a future Chilkat major version.
	// Applications should instead call the email object's SetFromXmlText method.
	// 
	...
	// This deprecated method will be removed in a future Chilkat major version.
	// Applications should instead call the email bundle object's LoadXml method.
	// 
	...
	// This deprecated method will be removed in a future Chilkat major version.
	// Applications should instead call the email bundle object's LoadXmlString method.
	// 
	...
	// This deprecated method will be removed in a future Chilkat major version.
	// Applications should instead use the Chilkat Dns class to do MX lookups.
	// 
	...
	// to be used in sending the command (such as utf-8 , ansi , iso-8859-1 , Shift_JIS
	// , etc.)
	...
	// encrypted, values substituted for replacement patterns, and header fields Q or B
	// encoded as needed based on the email. The RenderToMime method performs the
	...
	// 
	// References:
	// 1: https://en.wikipedia.org/wiki/List_of_SMTP_server_return_codes
	...
	// command. The recipients are the email addresses passed in RCPT TO commands. These are
	// usually the same email addresses found in the MIME headers, but need not be
	...
	// can be pop3 or smtp . When the protocol is pop3 , this is equivalent to setting the
	// PopPassword property. When protocol is smtp , this is equivalent to setting the
	...
	// http://www.cknotes.com/?p=249>http://www.cknotes.com/?p=249
	// 
	// 
	// References:
	// 1: <code
