...
class CK_VISIBLE_PUBLIC CkPdfW  : public CkClassWithCallbacksW
{
	...
	// The PDF owner password, if required.
	const wchar_t *ownerPassword(void);
	// The PDF owner password, if required.
	void put_OwnerPassword(const wchar_t *newVal);
	...
	// The PDF user password, if required.
	const wchar_t *userPassword(void);
	// The PDF user password, if required.
	void put_UserPassword(const wchar_t *newVal);
	...
	bool UpdateMetadata(CkStringBuilderW &sb, const wchar_t *outFilePath);

	// Updates or adds XMP metdata to a PDF. The metadata is passed in sb. If
	// successful, the PDF with updated or inserted metadata is written to bd.
	bool UpdateMetadataBd(CkStringBuilderW &sb, CkBinDataW &bd);
