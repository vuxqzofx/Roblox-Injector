...
class CK_VISIBLE_PUBLIC CkFileAccess  : public CkMultiByteBase
{
	...
	static CkFileAccess *createNew2(int progLang);
	...
	//     3. Access denied.
	//     4. File not found.
	//     5. General (non-specific) open error.
	//     6. File aleady exists.
	//     7. Path refers to a directory and the access requested involves writing.
	//     8. Too many symbolic links were encountered in resolving path.
	//     9. The process already has the maximum number of files open.
	//     10. Pathname is too long.
	//     11. The system limit on the total number of open files has been reached.
	//     12. Pathname refers to a device special file and no corresponding device
	//     exists.
	//     13. Insufficient kernel memory was available.
	//     14. Pathname was to be created but the device containing pathname has no
	//     room for the new file.
	//     15. A component used as a directory in pathname is not, in fact, a
	//     directory.
	//     16. Pathname refers to a regular file, too large to be opened (this would be
	//     a limitation of the underlying operating system, not a limitation imposed by
	//     Chilkat).
	//     17. Pathname refers to a file on a read-only filesystem and write access was
	//     requested.
	// 
	...
	// Returns 1 if the path exists, 0 if the path does not exist, and -1 if unable to
	...
	// example, if index = 8, length = 12, and encoding = base64 , then the string 00000012 is
	...
	// Returns one of the following times for a file:0: Last-modified
	// 1: Last-access
	// 2: Creation
	// The path argument indicates which time to return. The values can be 0, 1, or 2.
	// 
	...
	// 
	bool GetFileTimeStr(const char *path, int which, CkString &outStr);
	...
	// The time is returned in the ISO 8601 timestamp format: YYYY-MM-DDTHH:MM:SSZ.
	// 
	const char *getFileTimeStr(const char *path, int which);
	// Returns one of the following times for a file:0: Last-modified
	// 1: Last-access
	// 2: Creation
	// The path argument indicates which time to return. The values can be 0, 1, or 2.
	...
	// The time is returned in the ISO 8601 timestamp format: YYYY-MM-DDTHH:MM:SSZ.
	// 
	const char *fileTimeStr(const char *path, int which);
	...
	// the beginMarker is a string such as "<abc>", then it will also match "<abc ", where a
	// whitespace char follows the tag. This is to handle cases where some XML tags
	// have attributes and others do not.
	// 
